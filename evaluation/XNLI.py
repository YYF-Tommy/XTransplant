...
def get_golden(lang):
    golden = []
    with jsonlines.open(f"/XTransplant/data/XNLI/XNLI_unseen/ans/{lang}.json", "r") as f:
        for line in f:
            golden.append(line)
    return golden
...
def eval(path, lang):
    with jsonlines.open(f"/XTransplant/data/XNLI/XNLI_unseen/ans_lang/{lang}.json") as f:
        for line in f:
            clean_tag = line
...
for model_name in ["Llama-2-7b-chat-hf", "Mistral-7B-Instruct-v0.3", "Qwen2-7B-Instruct", "chinese-alpaca-2-7b"]:
    all = []
    log = f""
    files = os.listdir(f"/XTransplant/output_attn/XNLI_unseen/{model_name}")
    files.sort()
    ...
    for file in files:
        ...
        lang = file.split(".")[0]
        acc, s, num = eval(f"/XTransplant/output_attn/XNLI_unseen/{model_name}/{file}", lang)
        acc_count += len(s)
        acc_num += num
    ...
    with open(f"/XTransplant/output_attn/XNLI_unseen/{model_name}/acc_log.txt", 'w') as f_w:
        f_w.write(log)
