...
def get_golden(lang):
    golden = []
    with jsonlines.open(f"/XTransplant/data/XCOPA/XCOPA_practice/ans/{lang}.json", "r") as f:
        for line in f:
            golden.append(line)
    return golden
...
for model_name in ["Llama-2-7b-chat-hf", "Mistral-7B-Instruct-v0.3", "Qwen2-7B-Instruct"]:
    ...
    all = []
    log = f"{model_name} + XCOPA_unseen + {mode}\n\n"
    files = os.listdir(f"/XTransplant/ApplyExp/output/XCOPA_unseen/{model_name}/{mode}")
    files.sort()
    ...
    for file in files:
        if file.endswith(".txt") or file.startswith("all"):
            continue
        lang = file.split(".")[0]
        acc, s, num = eval(f"/XTransplant/ApplyExp/output/XCOPA_unseen/{model_name}/{mode}/{file}", lang)
        acc_count += len(s)
        acc_num += num
    ...
    with open(f"/XTransplant/ApplyExp/output/XCOPA_unseen/{model_name}/{mode}/acc_log.txt", 'w') as f_w:
        f_w.write(log)
