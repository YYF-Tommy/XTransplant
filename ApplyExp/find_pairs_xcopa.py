...
def acc4lang(lang):
    ...
    for i in range(0, N):
        union_y = {}
        for j in range(0, N):
            acc, s = eval(f"/XTransplant/UpperBound/output/XCOPA_sample_reverse/{model_name}_all/transplant_{i}to{j}_firsttoken/{lang}.json", lang)
            all[i, j] = s.copy()
    union = []
    for i in range(0, N):
        union_x = []
...
for model_name in ["Llama-2-7b-chat-hf", "Qwen2-7B-Instruct", "Mistral-7B-Instruct-v0.3"]:
    ...
    os.makedirs(f"/XTransplant/ApplyExp/saved_pairs/XCOPA_sample_reverse/{mode}", exist_ok=True)
    with jsonlines.open(f"/XTransplant/ApplyExp/saved_pairs/XCOPA_sample_reverse/{mode}/{model_name}.json", 'w') as f:
        f.write(d)